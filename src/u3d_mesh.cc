...
{
    uint32_t chain_index = reader.read<uint32_t>();    //Chain index is always zero.
    DUMP(chain_index);
    if (chain_index!=0) throw "Chain index must be 0";
    ...
    DUMP(position_quality);
    DUMP(normal_quality);
    DUMP(texcoord_quality);    
    DUMP(position_iq);
    DUMP(normal_iq);
    DUMP(texcoord_iq);
    DUMP(diffuse_iq);
